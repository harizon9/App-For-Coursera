...
function getTasksByTime(_time){
	var query = "SELECT * FROM TASKPROPS WHERE TASKPROP2 <= " + _time + " AND TASKPROP1 = '0';";
...
function getStatistics(){
	...
	return {totalNumberOfTasks:noOfTasks, totalTimeForTasks:timeForTasks};
...
function updateTask(inParam){
	...
	if(inParam.repeatable == 1){
		query = "UPDATE TASKPROPS SET TASKNAME = '" + inParam.taskName + "', TASKPROP1 = " + inParam.done + ", TASKPROP3 = '0', TASKPROP2 = '" + inParam.taskTime +
							"' WHERE TASKPROP12 = " + inParam.taskID + ";";
		conn.execute(query);
		
		query = "INSERT INTO TASKPROPS (TASKNAME, TASKPROP1, TASKPROP2, TASKPROP3) VALUES ('" + inParam.taskName + "','0'," + inParam.taskTime + ",'0');";
		conn.execute(query);
		
		
	} else {
		query = "UPDATE TASKPROPS SET TASKNAME = '" + inParam.taskName + "', TASKPROP1 = " + inParam.done + ", TASKPROP3 = " + inParam.repeatable + ", TASKPROP2 = '" + inParam.taskTime +
							"' WHERE TASKPROP12 = " + inParam.taskID + ";";
		conn.execute(query);
	}		
}
