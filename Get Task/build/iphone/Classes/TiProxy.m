...
//Rare exceptions to indicate a bug in the _gettask code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_gettask = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_gettask]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.gettask.module"
