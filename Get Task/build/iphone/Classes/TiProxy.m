...
//Rare exceptions to indicate a bug in the _t4t code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_t4t = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_t4t]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.t4t.module"
